...
void main() {
  runApp(MaterialApp(
    home: ElectronicBoardPage(),
  ));
}

// 게시판 화면
class ElectronicBoardPage extends StatefulWidget {
  @override
  _ElectronicBoardPageState createState() => _ElectronicBoardPageState();
}

const String fixMajorId = 'Electronic'; // 고정된 학번

class _ElectronicBoardPageState extends State<ElectronicBoardPage> {
  ...
  Future<void> addPost(String title, String content, String name) async {
    try {
      final response = await BoardApi.createMajorBoard(
          'Electronic', accessToken!, title, content);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        elevation: 0.0,
        title: Text('전기전자공학과 게시판', style: TextStyle(color: Colors.black)),
        centerTitle: true,
        leading: IconButton(
              ...
              child: IconButton(
                ...
                onPressed: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => NewElectronicBoardPostPage(
                        onAddPost: addPost,
                      ),
                    ),
                  );
...
// 글 등록 페이지
class NewElectronicBoardPostPage extends StatelessWidget {
  final Function(String, String, String) onAddPost;
  final TextEditingController titleController = TextEditingController();
  final TextEditingController contentController = TextEditingController();

  NewElectronicBoardPostPage({required this.onAddPost});
  ...
  Future<void> submitPost(BuildContext context) async {
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        // 1초 딜레이 후 ElectronicBoard게시판으로 이동
        Future.delayed(Duration(seconds: 1), () {
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
              builder: (context) => ElectronicBoardPage(),
            ),
          );
        });
      } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        backgroundColor: Colors.white,
        elevation: 0.0,
        title: Text('전기전장공학학과 게시판', style: TextStyle(color: Colors.black)),
        centerTitle: true,
        leading: IconButton(
          icon: Icon(Icons.arrow_back, color: Colors.black),
          onPressed: () => Navigator.pop(context),
...
// 글 상세 페이지
class PostElectronicBoardDetailPage extends StatefulWidget {
  final Map<String, String> post;

  PostElectronicBoardDetailPage({required this.post});

  @override
  _PostElectronicBoardDetailPageState createState() =>
      _PostElectronicBoardDetailPageState();
}

class _PostElectronicBoardDetailPageState
    extends State<PostElectronicBoardDetailPage> {
  ...
  Future<void> fetchPostDetails() async {
    ...
    try {
      final response =
          await BoardApi.getMajorBoardDetail('Electronic', postId, accessToken);
