...
void main() {
  runApp(MaterialApp(
    home: UndeclaredBoardPage(),
  ));
}

// 게시판 화면
class UndeclaredBoardPage extends StatefulWidget {
  @override
  _UndeclaredBoardPageState createState() => _UndeclaredBoardPageState();
}

const String fixMajorId = 'Undeclared'; // 고정된 학번

class _UndeclaredBoardPageState extends State<UndeclaredBoardPage> {
  ...
  Future<void> addPost(String title, String content, String name) async {
    try {
      final response = await BoardApi.createMajorBoard(
          'Undeclared', accessToken!, title, content);
              ...
              child: IconButton(
                ...
                onPressed: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => NewUndeclaredBoardPostPage(
                        onAddPost: addPost,
                      ),
                    ),
                  );
...
// 글 등록 페이지
class NewUndeclaredBoardPostPage extends StatelessWidget {
  final Function(String, String, String) onAddPost;
  final TextEditingController titleController = TextEditingController();
  final TextEditingController contentController = TextEditingController();

  NewUndeclaredBoardPostPage({required this.onAddPost});
  ...
  Future<void> submitPost(BuildContext context) async {
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        // 1초 딜레이 후 UndeclaredBoard게시판으로 이동
        Future.delayed(Duration(seconds: 1), () {
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
              builder: (context) => UndeclaredBoardPage(),
            ),
          );
        });
      } else {
...
// 글 상세 페이지
class PostUndeclaredBoardDetailPage extends StatefulWidget {
  final Map<String, String> post;

  PostUndeclaredBoardDetailPage({required this.post});

  @override
  _PostUndeclaredBoardDetailPageState createState() =>
      _PostUndeclaredBoardDetailPageState();
}

class _PostUndeclaredBoardDetailPageState
    extends State<PostUndeclaredBoardDetailPage> {
  ...
  Future<void> fetchPostDetails() async {
    ...
    try {
      final response =
          await BoardApi.getMajorBoardDetail('Undeclared', postId, accessToken);
