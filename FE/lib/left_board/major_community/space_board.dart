...
// 게시판 화면
class SpaceBoardPage extends StatefulWidget {
  @override
  _SpaceBoardPageState createState() => _SpaceBoardPageState();
}

const String fixMajorId = 'Space'; // 고정된 학번

class _SpaceBoardPageState extends State<SpaceBoardPage> {
  ...
  Future<void> addPost(String title, String content, String name) async {
    try {
      final response = await BoardApi.createMajorBoard(
          'Space', accessToken!, title, content);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        elevation: 0.0,
        title: Text('항공우주공학과 게시판', style: TextStyle(color: Colors.black)),
        centerTitle: true,
        leading: IconButton(
                      ...
                      return GestureDetector(
                        onTap: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => PostSpaceBoardDetailPage(
              ...
              child: IconButton(
                ...
                onPressed: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => NewSpaceBoardPostPage(
                        onAddPost: addPost,
                      ),
                    ),
                  );
...
// 글 등록 페이지
class NewSpaceBoardPostPage extends StatelessWidget {
  final Function(String, String, String) onAddPost;
  final TextEditingController titleController = TextEditingController();
  final TextEditingController contentController = TextEditingController();

  NewSpaceBoardPostPage({required this.onAddPost});
  ...
  Future<void> submitPost(BuildContext context) async {
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        // 1초 딜레이 후 SpaceBoard게시판으로 이동
        Future.delayed(Duration(seconds: 1), () {
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
              builder: (context) => SpaceBoardPage(),
            ),
          );
        });
      } else {
...
// 글 상세 페이지
class PostSpaceBoardDetailPage extends StatefulWidget {
  final Map<String, String> post;

  PostSpaceBoardDetailPage({required this.post});

  @override
  _PostSpaceBoardDetailPageState createState() =>
      _PostSpaceBoardDetailPageState();
}

class _PostSpaceBoardDetailPageState extends State<PostSpaceBoardDetailPage> {
  ...
  Future<void> fetchPostDetails() async {
    ...
    try {
      final response =
          await BoardApi.getMajorBoardDetail('Space', postId, accessToken);
