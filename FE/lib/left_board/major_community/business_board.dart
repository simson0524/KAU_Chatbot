...
const String fixMajorId = 'Business'; // 고정된 학번
  ...
  Future<void> addPost(String title, String content, String name) async {
    try {
      final response = await BoardApi.createMajorBoard(
          'Business', accessToken!, title, content);
                      ...
                      return GestureDetector(
                        onTap: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => PostBusinessBoardDetailPage(
              ...
              child: IconButton(
                ...
                onPressed: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => NewBusinessBoardPostPage(
                        onAddPost: addPost,
                      ),
                    ),
                  );
...
// 글 등록 페이지
class NewBusinessBoardPostPage extends StatelessWidget {
  final Function(String, String, String) onAddPost;
  final TextEditingController titleController = TextEditingController();
  final TextEditingController contentController = TextEditingController();

  NewBusinessBoardPostPage({required this.onAddPost});
  ...
  Future<void> submitPost(BuildContext context) async {
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        // 1초 딜레이 후 BusinessBoard게시판으로 이동
        Future.delayed(Duration(seconds: 1), () {
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
              builder: (context) => BusinessBoardPage(),
            ),
          );
        });
      } else {
...
// 글 상세 페이지
class PostBusinessBoardDetailPage extends StatefulWidget {
  final Map<String, String> post;

  PostBusinessBoardDetailPage({required this.post});

  @override
  _PostBusinessBoardDetailPageState createState() =>
      _PostBusinessBoardDetailPageState();
}

class _PostBusinessBoardDetailPageState
    extends State<PostBusinessBoardDetailPage> {
  ...
  Future<void> fetchPostDetails() async {
    ...
    try {
      final response =
          await BoardApi.getMajorBoardDetail('Business', postId, accessToken);
