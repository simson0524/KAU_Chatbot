...
void main() {
  runApp(MaterialApp(
    home: AirbusinessBoardPage(),
  ));
}

// 게시판 화면
class AirbusinessBoardPage extends StatefulWidget {
  @override
  _AirbusinessBoardPageState createState() => _AirbusinessBoardPageState();
}

const String fixMajorId = 'Airbusiness'; // 고정된 학번

class _AirbusinessBoardPageState extends State<AirbusinessBoardPage> {
  ...
  Future<void> addPost(String title, String content, String name) async {
    try {
      final response = await BoardApi.createMajorBoard(
          'Airbusiness', accessToken!, title, content);
                      ...
                      return GestureDetector(
                        onTap: () {
                          Navigator.push(
                            ...
                            MaterialPageRoute(
                              builder: (context) =>
                                  PostAirbusinessBoardDetailPage(
              ...
              child: IconButton(
                ...
                onPressed: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => NewAirbusinessBoardPostPage(
                        onAddPost: addPost,
                      ),
                    ),
                  );
...
// 글 등록 페이지
class NewAirbusinessBoardPostPage extends StatelessWidget {
  final Function(String, String, String) onAddPost;
  final TextEditingController titleController = TextEditingController();
  final TextEditingController contentController = TextEditingController();

  NewAirbusinessBoardPostPage({required this.onAddPost});
  ...
  Future<void> submitPost(BuildContext context) async {
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        // 1초 딜레이 후 AirbusinessBoard게시판으로 이동
        Future.delayed(Duration(seconds: 1), () {
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
              builder: (context) => AirbusinessBoardPage(),
            ),
          );
        });
      } else {
...
// 글 상세 페이지
class PostAirbusinessBoardDetailPage extends StatefulWidget {
  final Map<String, String> post;

  PostAirbusinessBoardDetailPage({required this.post});

  @override
  _PostAirbusinessBoardDetailPageState createState() =>
      _PostAirbusinessBoardDetailPageState();
}

class _PostAirbusinessBoardDetailPageState
    extends State<PostAirbusinessBoardDetailPage> {
  ...
  Future<void> fetchPostDetails() async {
    ...
    try {
      final response = await BoardApi.getMajorBoardDetail(
          'Airbusiness', postId, accessToken);
