...
void main() {
  runApp(MaterialApp(
    home: SmartdroneBoardPage(),
  ));
}

// 게시판 화면
class SmartdroneBoardPage extends StatefulWidget {
  @override
  _SmartdroneBoardPageState createState() => _SmartdroneBoardPageState();
}

const String fixMajorId = 'Smartdrone'; // 고정된 학번

class _SmartdroneBoardPageState extends State<SmartdroneBoardPage> {
  ...
  Future<void> addPost(String title, String content, String name) async {
    try {
      final response = await BoardApi.createMajorBoard(
          'Smartdrone', accessToken!, title, content);
              ...
              child: IconButton(
                ...
                onPressed: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => NewSmartdroneBoardPostPage(
                        onAddPost: addPost,
                      ),
                    ),
                  );
...
// 글 등록 페이지
class NewSmartdroneBoardPostPage extends StatelessWidget {
  final Function(String, String, String) onAddPost;
  final TextEditingController titleController = TextEditingController();
  final TextEditingController contentController = TextEditingController();

  NewSmartdroneBoardPostPage({required this.onAddPost});
  ...
  Future<void> submitPost(BuildContext context) async {
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        // 1초 딜레이 후 SmartdroneBoard게시판으로 이동
        Future.delayed(Duration(seconds: 1), () {
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
              builder: (context) => SmartdroneBoardPage(),
            ),
          );
        });
      } else {
...
// 글 상세 페이지
class PostSmartdroneBoardDetailPage extends StatefulWidget {
  final Map<String, String> post;

  PostSmartdroneBoardDetailPage({required this.post});

  @override
  _PostSmartdroneBoardDetailPageState createState() =>
      _PostSmartdroneBoardDetailPageState();
}

class _PostSmartdroneBoardDetailPageState
    extends State<PostSmartdroneBoardDetailPage> {
  ...
  Future<void> fetchPostDetails() async {
    ...
    try {
      final response =
          await BoardApi.getMajorBoardDetail('Smartdrone', postId, accessToken);
