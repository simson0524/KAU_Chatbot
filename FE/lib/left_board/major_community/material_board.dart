...
void main() {
  runApp(MaterialApp(
    home: MaterialBoardPage(),
  ));
}

// 게시판 화면
class MaterialBoardPage extends StatefulWidget {
  @override
  _MaterialBoardPageState createState() => _MaterialBoardPageState();
}

const String fixMajorId = 'Material'; // 고정된 학번

class _MaterialBoardPageState extends State<MaterialBoardPage> {
  ...
  Future<void> addPost(String title, String content, String name) async {
    try {
      final response = await BoardApi.createMajorBoard(
          'Material', accessToken!, title, content);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        elevation: 0.0,
        title: Text('신소재공학과 게시판', style: TextStyle(color: Colors.black)),
        centerTitle: true,
        leading: IconButton(
                      ...
                      return GestureDetector(
                        onTap: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => PostMaterialBoardDetailPage(
              ...
              child: IconButton(
                ...
                onPressed: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => NewMaterialBoardPostPage(
                        onAddPost: addPost,
                      ),
                    ),
                  );
...
// 글 등록 페이지
class NewMaterialBoardPostPage extends StatelessWidget {
  final Function(String, String, String) onAddPost;
  final TextEditingController titleController = TextEditingController();
  final TextEditingController contentController = TextEditingController();

  NewMaterialBoardPostPage({required this.onAddPost});
  ...
  Future<void> submitPost(BuildContext context) async {
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        // 1초 딜레이 후 MaterialBoard게시판으로 이동
        Future.delayed(Duration(seconds: 1), () {
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
              builder: (context) => MaterialBoardPage(),
            ),
          );
        });
      } else {
...
// 글 상세 페이지
class PostMaterialBoardDetailPage extends StatefulWidget {
  final Map<String, String> post;

  PostMaterialBoardDetailPage({required this.post});

  @override
  _PostMaterialBoardDetailPageState createState() =>
      _PostMaterialBoardDetailPageState();
}

class _PostMaterialBoardDetailPageState
    extends State<PostMaterialBoardDetailPage> {
  ...
  Future<void> fetchPostDetails() async {
    ...
    try {
      final response =
          await BoardApi.getMajorBoardDetail('Material', postId, accessToken);
