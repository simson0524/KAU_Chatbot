...
void main() {
  runApp(MaterialApp(
    home: InternationalBoardPage(),
  ));
}

// 게시판 화면
class InternationalBoardPage extends StatefulWidget {
  @override
  _InternationalBoardPageState createState() => _InternationalBoardPageState();
}

const String fixMajorId = 'International'; // 고정된 학번

class _InternationalBoardPageState extends State<InternationalBoardPage> {
  ...
  Future<void> addPost(String title, String content, String name) async {
    try {
      final response = await BoardApi.createMajorBoard(
          'International', accessToken!, title, content);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        elevation: 0.0,
        title: Text('국제교류학부 게시판', style: TextStyle(color: Colors.black)),
        centerTitle: true,
        leading: IconButton(
                      ...
                      return GestureDetector(
                        onTap: () {
                          Navigator.push(
                            ...
                            MaterialPageRoute(
                              builder: (context) =>
                                  PostInternationalBoardDetailPage(
              ...
              child: IconButton(
                ...
                onPressed: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => NewInternationalBoardPostPage(
                        onAddPost: addPost,
                      ),
                    ),
                  );
...
// 글 등록 페이지
class NewInternationalBoardPostPage extends StatelessWidget {
  final Function(String, String, String) onAddPost;
  final TextEditingController titleController = TextEditingController();
  final TextEditingController contentController = TextEditingController();

  NewInternationalBoardPostPage({required this.onAddPost});
  ...
  Future<void> submitPost(BuildContext context) async {
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        // 1초 딜레이 후 InternationalBoard게시판으로 이동
        Future.delayed(Duration(seconds: 1), () {
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
              builder: (context) => InternationalBoardPage(),
            ),
          );
        });
      } else {
...
// 글 상세 페이지
class PostInternationalBoardDetailPage extends StatefulWidget {
  final Map<String, String> post;

  PostInternationalBoardDetailPage({required this.post});

  @override
  _PostInternationalBoardDetailPageState createState() =>
      _PostInternationalBoardDetailPageState();
}

class _PostInternationalBoardDetailPageState
    extends State<PostInternationalBoardDetailPage> {
  ...
  Future<void> fetchPostDetails() async {
    ...
    try {
      final response = await BoardApi.getMajorBoardDetail(
          'International', postId, accessToken);
