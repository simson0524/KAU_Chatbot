...
void main() {
  runApp(MaterialApp(
    home: AvitationBoardPage(),
  ));
}

// 게시판 화면
class AvitationBoardPage extends StatefulWidget {
  @override
  _AvitationBoardPageState createState() => _AvitationBoardPageState();
}

const String fixMajorId = 'Avitation'; // 고정된 학번

class _AvitationBoardPageState extends State<AvitationBoardPage> {
  ...
  Future<void> addPost(String title, String content, String name) async {
    try {
      final response = await BoardApi.createMajorBoard(
          'Avitation', accessToken!, title, content);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        elevation: 0.0,
        title: Text('운항학과 게시판', style: TextStyle(color: Colors.black)),
        centerTitle: true,
        leading: IconButton(
              ...
              child: IconButton(
                ...
                onPressed: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => NewAvitationBoardPostPage(
                        onAddPost: addPost,
                      ),
                    ),
                  );
...
// 글 등록 페이지
class NewAvitationBoardPostPage extends StatelessWidget {
  final Function(String, String, String) onAddPost;
  final TextEditingController titleController = TextEditingController();
  final TextEditingController contentController = TextEditingController();

  NewAvitationBoardPostPage({required this.onAddPost});
  ...
  Future<void> submitPost(BuildContext context) async {
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        // 1초 딜레이 후 AvitationBoard게시판으로 이동
        Future.delayed(Duration(seconds: 1), () {
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
              builder: (context) => AvitationBoardPage(),
            ),
          );
        });
      } else {
...
// 글 상세 페이지
class PostAvitationBoardDetailPage extends StatefulWidget {
  final Map<String, String> post;

  PostAvitationBoardDetailPage({required this.post});

  @override
  _PostAvitationBoardDetailPageState createState() =>
      _PostAvitationBoardDetailPageState();
}

class _PostAvitationBoardDetailPageState
    extends State<PostAvitationBoardDetailPage> {
  ...
  Future<void> fetchPostDetails() async {
    ...
    try {
      final response =
          await BoardApi.getMajorBoardDetail('Avitation', postId, accessToken);
