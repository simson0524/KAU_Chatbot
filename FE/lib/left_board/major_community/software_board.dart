...
void main() {
  runApp(MaterialApp(
    home: SoftwareBoardPage(),
  ));
}

// 게시판 화면
class SoftwareBoardPage extends StatefulWidget {
  @override
  _SoftwareBoardPageState createState() => _SoftwareBoardPageState();
}

const String fixMajorId = 'Software'; // 고정된 학번

class _SoftwareBoardPageState extends State<SoftwareBoardPage> {
  ...
  Future<void> addPost(String title, String content, String name) async {
    try {
      final response = await BoardApi.createMajorBoard(
          'Software', accessToken!, title, content);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        elevation: 0.0,
        title: Text('소프트웨어학과 게시판', style: TextStyle(color: Colors.black)),
        centerTitle: true,
        leading: IconButton(
                      ...
                      return GestureDetector(
                        onTap: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => PostSoftwareBoardDetailPage(
              ...
              child: IconButton(
                ...
                onPressed: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => NewSoftwareBoardPostPage(
                        onAddPost: addPost,
                      ),
                    ),
                  );
...
// 글 등록 페이지
class NewSoftwareBoardPostPage extends StatelessWidget {
  final Function(String, String, String) onAddPost;
  final TextEditingController titleController = TextEditingController();
  final TextEditingController contentController = TextEditingController();

  NewSoftwareBoardPostPage({required this.onAddPost});
  ...
  Future<void> submitPost(BuildContext context) async {
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        // 1초 딜레이 후 SoftwareBoard게시판으로 이동
        Future.delayed(Duration(seconds: 1), () {
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
              builder: (context) => SoftwareBoardPage(),
            ),
          );
        });
      } else {
...
// 글 상세 페이지
class PostSoftwareBoardDetailPage extends StatefulWidget {
  final Map<String, String> post;

  PostSoftwareBoardDetailPage({required this.post});

  @override
  _PostSoftwareBoardDetailPageState createState() =>
      _PostSoftwareBoardDetailPageState();
}

class _PostSoftwareBoardDetailPageState
    extends State<PostSoftwareBoardDetailPage> {
  ...
  Future<void> fetchPostDetails() async {
    ...
    try {
      final response =
          await BoardApi.getMajorBoardDetail('Software', postId, accessToken);
