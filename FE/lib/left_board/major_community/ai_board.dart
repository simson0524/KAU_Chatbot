...
// 게시판 화면
class AiBoardPage extends StatefulWidget {
  @override
  _AiBoardPageState createState() => _AiBoardPageState();
}

const String fixMajorId = 'Ai'; // 고정된 학번

class _AiBoardPageState extends State<AiBoardPage> {
  ...
  Future<void> addPost(String title, String content, String name) async {
    try {
      final response =
          await BoardApi.createMajorBoard('Ai', accessToken!, title, content);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        elevation: 0.0,
        title: Text('AI자율주행시스템공학과 게시판', style: TextStyle(color: Colors.black)),
        centerTitle: true,
        leading: IconButton(
                      ...
                      return GestureDetector(
                        onTap: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => PostAiBoardDetailPage(
              ...
              child: IconButton(
                ...
                onPressed: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => NewAiBoardPostPage(
                        onAddPost: addPost,
                      ),
                    ),
                  );
...
// 글 등록 페이지
class NewAiBoardPostPage extends StatelessWidget {
  final Function(String, String, String) onAddPost;
  final TextEditingController titleController = TextEditingController();
  final TextEditingController contentController = TextEditingController();

  NewAiBoardPostPage({required this.onAddPost});
  ...
  Future<void> submitPost(BuildContext context) async {
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        // 1초 딜레이 후 AiBoard게시판으로 이동
        Future.delayed(Duration(seconds: 1), () {
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
              builder: (context) => AiBoardPage(),
            ),
          );
        });
      } else {
...
// 글 상세 페이지
class PostAiBoardDetailPage extends StatefulWidget {
  final Map<String, String> post;

  PostAiBoardDetailPage({required this.post});

  @override
  _PostAiBoardDetailPageState createState() => _PostAiBoardDetailPageState();
}

class _PostAiBoardDetailPageState extends State<PostAiBoardDetailPage> {
  ...
  Future<void> fetchPostDetails() async {
    ...
    try {
      final response =
          await BoardApi.getMajorBoardDetail('Ai', postId, accessToken);
