...
void main() {
  runApp(MaterialApp(
    home: EngineeringBoardPage(),
  ));
}

// 게시판 화면
class EngineeringBoardPage extends StatefulWidget {
  @override
  _EngineeringBoardPageState createState() => _EngineeringBoardPageState();
}

const String fixMajorId = 'Engineering'; // 고정된 학번

class _EngineeringBoardPageState extends State<EngineeringBoardPage> {
  ...
  Future<void> addPost(String title, String content, String name) async {
    try {
      final response = await BoardApi.createMajorBoard(
          'Engineering', accessToken!, title, content);
                      ...
                      return GestureDetector(
                        onTap: () {
                          Navigator.push(
                            ...
                            MaterialPageRoute(
                              builder: (context) =>
                                  PostEngineeringBoardDetailPage(
              ...
              child: IconButton(
                ...
                onPressed: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => NewEngineeringBoardPostPage(
                        onAddPost: addPost,
                      ),
                    ),
                  );
...
// 글 등록 페이지
class NewEngineeringBoardPostPage extends StatelessWidget {
  final Function(String, String, String) onAddPost;
  final TextEditingController titleController = TextEditingController();
  final TextEditingController contentController = TextEditingController();

  NewEngineeringBoardPostPage({required this.onAddPost});
  ...
  Future<void> submitPost(BuildContext context) async {
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        // 1초 딜레이 후 EngineeringBoard게시판으로 이동
        Future.delayed(Duration(seconds: 1), () {
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
              builder: (context) => EngineeringBoardPage(),
            ),
          );
        });
      } else {
...
// 글 상세 페이지
class PostEngineeringBoardDetailPage extends StatefulWidget {
  final Map<String, String> post;

  PostEngineeringBoardDetailPage({required this.post});

  @override
  _PostEngineeringBoardDetailPageState createState() =>
      _PostEngineeringBoardDetailPageState();
}

class _PostEngineeringBoardDetailPageState
    extends State<PostEngineeringBoardDetailPage> {
  ...
  Future<void> fetchPostDetails() async {
    ...
    try {
      final response = await BoardApi.getMajorBoardDetail(
          'Engineering', postId, accessToken);
