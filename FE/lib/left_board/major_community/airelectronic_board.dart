...
const String fixMajorId = 'Airelectronic'; // 고정된 학번
  ...
  Future<void> addPost(String title, String content, String name) async {
    try {
      final response = await BoardApi.createMajorBoard(
          'Airelectronic', accessToken!, title, content);
                      ...
                      return GestureDetector(
                        onTap: () {
                          Navigator.push(
                            ...
                            MaterialPageRoute(
                              builder: (context) =>
                                  PostAirelectronicBoardDetailPage(
              ...
              child: IconButton(
                ...
                onPressed: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => NewAirelectronicBoardPostPage(
                        onAddPost: addPost,
                      ),
                    ),
                  );
...
// 글 등록 페이지
class NewAirelectronicBoardPostPage extends StatelessWidget {
  final Function(String, String, String) onAddPost;
  final TextEditingController titleController = TextEditingController();
  final TextEditingController contentController = TextEditingController();

  NewAirelectronicBoardPostPage({required this.onAddPost});
  ...
  Future<void> submitPost(BuildContext context) async {
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        // 1초 딜레이 후 AirelectronicBoard게시판으로 이동
        Future.delayed(Duration(seconds: 1), () {
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
              builder: (context) => AirelectronicBoardPage(),
            ),
          );
        });
      } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        backgroundColor: Colors.white,
        elevation: 0.0,
        title: Text('항공전자정보학부 게시판', style: TextStyle(color: Colors.black)),
        centerTitle: true,
        leading: IconButton(
          icon: Icon(Icons.arrow_back, color: Colors.black),
          onPressed: () => Navigator.pop(context),
...
// 글 상세 페이지
class PostAirelectronicBoardDetailPage extends StatefulWidget {
  final Map<String, String> post;

  PostAirelectronicBoardDetailPage({required this.post});

  @override
  _PostAirelectronicBoardDetailPageState createState() =>
      _PostAirelectronicBoardDetailPageState();
}

class _PostAirelectronicBoardDetailPageState
    extends State<PostAirelectronicBoardDetailPage> {
  ...
  Future<void> fetchPostDetails() async {
    ...
    try {
      final response = await BoardApi.getMajorBoardDetail(
          'Airelectronic', postId, accessToken);
