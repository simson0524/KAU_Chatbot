...
const String fixMajorId = 'Spaceengineering'; // 고정된 학번
  ...
  Future<void> addPost(String title, String content, String name) async {
    try {
      final response = await BoardApi.createMajorBoard(
          'Spaceengineering', accessToken!, title, content);
                      ...
                      return GestureDetector(
                        onTap: () {
                          Navigator.push(
                            ...
                            MaterialPageRoute(
                              builder: (context) =>
                                  PostSpaceengineeringBoardDetailPage(
              ...
              child: IconButton(
                ...
                onPressed: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => NewSpaceengineeringBoardPostPage(
                        onAddPost: addPost,
                      ),
                    ),
                  );
...
// 글 등록 페이지
class NewSpaceengineeringBoardPostPage extends StatelessWidget {
  final Function(String, String, String) onAddPost;
  final TextEditingController titleController = TextEditingController();
  final TextEditingController contentController = TextEditingController();

  NewSpaceengineeringBoardPostPage({required this.onAddPost});
  ...
  Future<void> submitPost(BuildContext context) async {
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        // 1초 딜레이 후 SpaceengineeringBoard게시판으로 이동
        Future.delayed(Duration(seconds: 1), () {
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
              builder: (context) => SpaceengineeringBoardPage(),
            ),
          );
        });
      } else {
...
// 글 상세 페이지
class PostSpaceengineeringBoardDetailPage extends StatefulWidget {
  final Map<String, String> post;

  PostSpaceengineeringBoardDetailPage({required this.post});

  @override
  _PostSpaceengineeringBoardDetailPageState createState() =>
      _PostSpaceengineeringBoardDetailPageState();
}

class _PostSpaceengineeringBoardDetailPageState
    extends State<PostSpaceengineeringBoardDetailPage> {
  ...
  Future<void> fetchPostDetails() async {
    ...
    try {
      final response = await BoardApi.getMajorBoardDetail(
          'Spaceengineering', postId, accessToken);
