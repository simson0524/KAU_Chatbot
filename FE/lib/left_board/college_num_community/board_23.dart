...
// 게시판 화면
class Board23Page extends StatefulWidget {
  @override
  _Board23PageState createState() => _Board23PageState();
}

const int fixStudentId = 2023; // 고정된 학번

class _Board23PageState extends State<Board23Page> {
  ...
  Future<void> addPost(String title, String content, String name) async {
    try {
      final response =
          await BoardApi.createStudentBoard(2023, accessToken!, title, content);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        elevation: 0.0,
        title: Text('23학번 게시판', style: TextStyle(color: Colors.black)),
        centerTitle: true,
        leading: IconButton(
                      ...
                      return GestureDetector(
                        onTap: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => Post23DetailPage(
...
// 글 등록 페이지
class New23PostPage extends StatelessWidget {
  final Function(String, String, String) onAddPost;
  final TextEditingController titleController = TextEditingController();
  final TextEditingController contentController = TextEditingController();

  New23PostPage({required this.onAddPost});
  ...
  Future<void> submitPost(BuildContext context) async {
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        // 1초 딜레이 후 23학번 게시판으로 이동
        Future.delayed(Duration(seconds: 1), () {
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
              builder: (context) => Board23Page(),
            ),
          );
        });
      } else {
...
// 글 상세 페이지
class Post23DetailPage extends StatefulWidget {
  final Map<String, String> post;

  Post23DetailPage({required this.post});

  @override
  _Post23DetailPageState createState() => _Post23DetailPageState();
}

class _Post23DetailPageState extends State<Post23DetailPage> {
  ...
  Future<void> fetchPostDetails() async {
    ...
    try {
      final response =
          await BoardApi.getStudentBoardDetail(2023, postId, accessToken);
