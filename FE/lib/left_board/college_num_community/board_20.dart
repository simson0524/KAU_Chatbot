...
// 게시판 화면
class Board20Page extends StatefulWidget {
  @override
  _Board20PageState createState() => _Board20PageState();
}

const int fixStudentId = 2020; // 고정된 학번

class _Board20PageState extends State<Board20Page> {
  ...
  Future<void> addPost(String title, String content, String name) async {
    try {
      final response =
          await BoardApi.createStudentBoard(2020, accessToken!, title, content);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        elevation: 0.0,
        title: Text('20학번 게시판', style: TextStyle(color: Colors.black)),
        centerTitle: true,
        leading: IconButton(
                      ...
                      return GestureDetector(
                        onTap: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => Post20DetailPage(
...
// 글 등록 페이지
class New20PostPage extends StatelessWidget {
  final Function(String, String, String) onAddPost;
  final TextEditingController titleController = TextEditingController();
  final TextEditingController contentController = TextEditingController();

  New20PostPage({required this.onAddPost});
...
// 글 상세 페이지
class Post20DetailPage extends StatefulWidget {
  final Map<String, String> post;

  Post20DetailPage({required this.post});

  @override
  _Post20DetailPageState createState() => _Post20DetailPageState();
}

class _Post20DetailPageState extends State<Post20DetailPage> {
  ...
  Future<void> fetchPostDetails() async {
    ...
    try {
      final response =
          await BoardApi.getStudentBoardDetail(2020, postId, accessToken);
