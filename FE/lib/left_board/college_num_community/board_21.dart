...
// 게시판 화면
class Board21Page extends StatefulWidget {
  @override
  _Board21PageState createState() => _Board21PageState();
}

const int fixStudentId = 2021; // 고정된 학번

class _Board21PageState extends State<Board21Page> {
  ...
  Future<void> addPost(String title, String content, String name) async {
    try {
      final response =
          await BoardApi.createStudentBoard(2021, accessToken!, title, content);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        elevation: 0.0,
        title: Text('21학번 게시판', style: TextStyle(color: Colors.black)),
        centerTitle: true,
        leading: IconButton(
                      ...
                      return GestureDetector(
                        onTap: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => Post21DetailPage(
...
class New21PostPage extends StatelessWidget {
  ...
  New21PostPage({required this.onAddPost});
  ...
  Future<void> submitPost(BuildContext context) async {
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        // 1초 딜레이 후 21학번 게시판으로 이동
        Future.delayed(Duration(seconds: 1), () {
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
              builder: (context) => Board21Page(),
            ),
          );
        });
      } else {
...
// 글 상세 페이지
class Post21DetailPage extends StatefulWidget {
  final Map<String, String> post;

  Post21DetailPage({required this.post});

  @override
  _Post21DetailPageState createState() => _Post21DetailPageState();
}

class _Post21DetailPageState extends State<Post21DetailPage> {
  ...
  Future<void> fetchPostDetails() async {
    ...
    try {
      final response =
          await BoardApi.getStudentBoardDetail(2021, postId, accessToken);
