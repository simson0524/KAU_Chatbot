...
// 게시판 화면
class Board24Page extends StatefulWidget {
  @override
  _Board24PageState createState() => _Board24PageState();
}

const int fixStudentId = 2024; // 고정된 학번

class _Board24PageState extends State<Board24Page> {
                      ...
                      return GestureDetector(
                        onTap: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => Post24DetailPage(
...
// 글 등록 페이지
class New24PostPage extends StatelessWidget {
  final Function(String, String, String) onAddPost;
  final TextEditingController titleController = TextEditingController();
  final TextEditingController contentController = TextEditingController();

  New24PostPage({required this.onAddPost});
  ...
  Future<void> submitPost(BuildContext context) async {
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        // 1초 딜레이 후 24학번 게시판으로 이동
        Future.delayed(Duration(seconds: 1), () {
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
              builder: (context) => Board24Page(),
            ),
          );
        });
      } else {
...
// 글 상세 페이지
class Post24DetailPage extends StatefulWidget {
  final Map<String, String> post;

  Post24DetailPage({required this.post});

  @override
  _Post24DetailPageState createState() => _Post24DetailPageState();
}

class _Post24DetailPageState extends State<Post24DetailPage> {
  ...
  Future<void> fetchPostDetails() async {
    ...
    try {
      final response =
          await BoardApi.getStudentBoardDetail(2024, postId, accessToken);
