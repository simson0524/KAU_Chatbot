...
// 게시판 화면
class Board22Page extends StatefulWidget {
  @override
  _Board22PageState createState() => _Board22PageState();
}

const int fixStudentId = 2022; // 고정된 학번

class _Board22PageState extends State<Board22Page> {
  ...
  Future<void> addPost(String title, String content, String name) async {
    try {
      final response =
          await BoardApi.createStudentBoard(2022, accessToken!, title, content);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        elevation: 0.0,
        title: Text('22학번 게시판', style: TextStyle(color: Colors.black)),
        centerTitle: true,
        leading: IconButton(
                      ...
                      return GestureDetector(
                        onTap: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => Post22DetailPage(
...
// 글 등록 페이지
class New22PostPage extends StatelessWidget {
  final Function(String, String, String) onAddPost;
  final TextEditingController titleController = TextEditingController();
  final TextEditingController contentController = TextEditingController();

  New22PostPage({required this.onAddPost});
  ...
  Future<void> submitPost(BuildContext context) async {
    ...
    try {
      ...
      // API를 통해 게시글 등록
      final response = await BoardApi.createStudentBoard(
        2022, // 학번을 고정값으로 사용
        accessToken,
      ...
      if (response.statusCode == 201) {
        ...
        // 1초 딜레이 후 22학번 게시판으로 이동
        Future.delayed(Duration(seconds: 1), () {
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
              builder: (context) => Board22Page(),
            ),
          );
        });
      } else {
...
// 글 상세 페이지
class Post22DetailPage extends StatefulWidget {
  final Map<String, String> post;

  Post22DetailPage({required this.post});

  @override
  _Post22DetailPageState createState() => _Post22DetailPageState();
}

class _Post22DetailPageState extends State<Post22DetailPage> {
  ...
  Future<void> fetchPostDetails() async {
    ...
    try {
      final response =
          await BoardApi.getStudentBoardDetail(2022, postId, accessToken);
  ...
  Future<void> submitComment() async {
    ...
    try {
      final response = await BoardApi.addComment(
        2022, // 학번
        postId,
        accessToken,
