...
// 게시판 화면
class Board18Page extends StatefulWidget {
  @override
  _Board18PageState createState() => _Board18PageState();
}

const int fixStudentId = 2018; // 고정된 학번

class _Board18PageState extends State<Board18Page> {
  ...
  Future<void> addPost(String title, String content, String name) async {
    try {
      final response =
          await BoardApi.createStudentBoard(2018, accessToken!, title, content);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        elevation: 0.0,
        title: Text('18학번 게시판', style: TextStyle(color: Colors.black)),
        centerTitle: true,
        leading: IconButton(
                      ...
                      return GestureDetector(
                        onTap: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => Post18DetailPage(
...
// 글 등록 페이지
class New18PostPage extends StatelessWidget {
  final Function(String, String, String) onAddPost;
  final TextEditingController titleController = TextEditingController();
  final TextEditingController contentController = TextEditingController();

  New18PostPage({required this.onAddPost});
  ...
  Future<void> submitPost(BuildContext context) async {
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        // 1초 딜레이 후 18학번 게시판으로 이동
        Future.delayed(Duration(seconds: 1), () {
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
              builder: (context) => Board18Page(),
            ),
          );
        });
      } else {
...
// 글 상세 페이지
class Post18DetailPage extends StatefulWidget {
  final Map<String, String> post;

  Post18DetailPage({required this.post});

  @override
  _Post18DetailPageState createState() => _Post18DetailPageState();
}

class _Post18DetailPageState extends State<Post18DetailPage> {
  ...
  Future<void> fetchPostDetails() async {
    ...
    try {
      final response =
          await BoardApi.getStudentBoardDetail(2018, postId, accessToken);
