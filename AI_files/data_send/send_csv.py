...
rec_url = "http://3.38.42.182:80/csv"
...
try:
    ...
    # 원본 CSV 파일 전송 (두 번째 URL)
    with open(input_file_path, 'rb') as file:
        files = {'file': ('original_contest_data.csv', file)}
        response2 = requests.post(rec_url, files=files)

    # 서버 응답 확인 (두 번째 URL)
    if response2.status_code == 200:
        print("파일 업로드 성공! (원본 CSV)")
        print(response2.json())
    else:
        print(f"파일 업로드 실패 (원본 CSV). 상태 코드: {response2.status_code}")
